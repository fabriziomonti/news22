	...
	<script type='text/javascript'>
		...
		function addRowContextMenuWhenLoaded(tblName, rowId)
			...
			<xsl:if test="/watabella/versione_librerie_xsl = '10117'">
				if (myDomReady == 1 <xsl:value-of select="string('&amp;&amp;')" /> typeof addRowContextMenu == 'function')
			</xsl:if>
			<xsl:if test="/watabella/versione_librerie_xsl != '10117'">
				if (myDomReady == 1 <xsl:value-of disable-output-escaping="yes" select="string('&amp;&amp;')" /> typeof addRowContextMenu == 'function')
			</xsl:if>
